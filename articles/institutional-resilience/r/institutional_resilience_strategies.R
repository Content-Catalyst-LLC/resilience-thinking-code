# Institutional resilience strategy scoring workflow.
# Run: Rscript r/institutional_resilience_strategies.R
...
strategies_path <- file.path(root, "data", "raw", "institutional_resilience_strategies.csv")
scenarios_path <- file.path(root, "data", "raw", "institutional_resilience_scenarios.csv")
...
score_strategies <- function(data, scenario_row) {
  data %>%
    mutate(
      ...
      threshold_risk = pmax(0, 8.0 - pmin(legitimacy, capacity, coordination, accountability, equity_protection)),
...
ggsave(
  filename = file.path(out_figures, "r_institutional_resilience_strategy_rankings.png"),
  plot = p1,
  width = 10,
  height = 6,
  dpi = 160
...
write_csv(ranked_results, file.path(out_tables, "r_institutional_resilience_strategy_rankings.csv"))
write_csv(top_rank_summary, file.path(out_tables, "r_institutional_resilience_top_rank_summary.csv"))
write_csv(profiles, file.path(out_tables, "r_institutional_resilience_strategy_profiles.csv"))
