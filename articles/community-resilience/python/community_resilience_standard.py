#!/usr/bin/env python3
# Dependency-light Community Resilience workflow.
# Run: python3 python/community_resilience_standard.py
...
def base_resilience_value(row: dict[str, str]) -> float:
    return (
        0.14 * f(row, "social_capital")
        + 0.14 * f(row, "institutional_capacity")
        + 0.14 * f(row, "infrastructure_access")
        + 0.13 * f(row, "economic_security")
        + 0.13 * f(row, "information_quality")
...
def score_strategy(row: dict[str, str], scenario: dict[str, str]) -> float:
    return (
        f(scenario, "social_capital_weight") * f(row, "social_capital")
        + f(scenario, "institutional_capacity_weight") * f(row, "institutional_capacity")
        + f(scenario, "infrastructure_access_weight") * f(row, "infrastructure_access")
        + f(scenario, "economic_security_weight") * f(row, "economic_security")
        + f(scenario, "information_quality_weight") * f(row, "information_quality")
...
def strategy_diagnostic(row: dict[str, str], value: float) -> str:
    if value >= 8.10 and f(row, "implementation_burden") <= 3.0:
        return "strong community resilience profile with manageable implementation burden"
    ...
    if f(row, "infrastructure_access") < 7.4:
        return "infrastructure access needs strengthening"
    if f(row, "information_quality") < 7.6:
...
def simulate_community_response(system, events, seed, time_steps=80):
    rng = random.Random(seed)
    function_level = f(system, "baseline_function")
    social = f(system, "social_capital")
    institution = f(system, "institutional_capacity")
    access = f(system, "infrastructure_access")
    economy = f(system, "economic_security")
    information = f(system, "information_quality")
    ...
    for t in range(time_steps):
        ...
        if event:
            ...
            service = f(event, "service_disruption")
            institutional_pressure = f(event, "institutional_pressure")
            communication = f(event, "communication_stress")
            economic = f(event, "economic_shock")
            care = f(event, "care_burden")
            ...
            event_name = "background community stress"
        ...
        rows.append({
            ...
            "community_function": round(function_level, 5),
...
def summarize_simulation(rows):
    ...
    for system in sorted(set(str(row["system"]) for row in rows)):
        ...
        function_values = [float(row["community_function"]) for row in subset]
        equity = [float(row["equity_adjusted_function"]) for row in subset]
        dependency = [float(row["dependency_coupling"]) for row in subset]
        summary.append({
            "system": system,
            "mean_community_function": round(mean(function_values), 5),
            "minimum_community_function": round(min(function_values), 5),
            "final_community_function": round(function_values[-1], 5),
...
def main() -> None:
    strategies = read_csv(ROOT / "data/raw/community_resilience_strategies.csv")
    scenarios = read_csv(ROOT / "data/raw/community_resilience_scenarios.csv")
    systems = read_csv(ROOT / "data/raw/community_systems.csv")
    events = read_csv(ROOT / "data/raw/community_stress_events.csv")
    ...
    for row in strategies:
        ...
        strategy_profiles.append({
            ...
            "social_capital": row["social_capital"],
            "institutional_capacity": row["institutional_capacity"],
            "infrastructure_access": row["infrastructure_access"],
            "economic_security": row["economic_security"],
    ...
    for idx, system in enumerate(systems):
        dynamic_rows.extend(simulate_community_response(system, events, seed=100 + idx))
    ...
    write_csv(OUT_TABLES / "community_resilience_strategy_profiles_standard.csv", strategy_profiles)
    write_csv(OUT_TABLES / "community_resilience_strategy_rankings_standard.csv", rankings)
    write_csv(OUT_TABLES / "community_resilience_top_rank_summary_standard.csv", top_rank_rows)
    write_csv(OUT_TABLES / "community_dynamic_simulation_standard.csv", dynamic_rows)
    write_csv(OUT_TABLES / "community_dynamic_summary_standard.csv", dynamic_summary)
    write_csv(OUT_TABLES / "community_resilience_monte_carlo_standard.csv", strategy_simulation)
    write_csv(OUT_TABLES / "community_resilience_robustness_summary_standard.csv", robustness_rows)
    write_csv(DATA_PROCESSED / "community_resilience_strategy_profiles_standard.csv", strategy_profiles)

    print("Community resilience workflow complete.")
