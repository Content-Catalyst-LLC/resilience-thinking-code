#!/usr/bin/env python3
"""
Advanced climate resilience workflow.
...
def calculate_strategy_profiles(strategies: pd.DataFrame) -> pd.DataFrame:
    ...
    out["base_resilience_value"] = (
        ...
        + 0.16 * out["vulnerability_reduction"]
        + 0.16 * out["adaptive_capacity"]
        + 0.15 * out["recovery_capacity"]
        + 0.15 * out["transformative_capacity"]
        + 0.14 * out["justice_protection"]
        - 0.08 * out["maladaptation_risk"]
    )
    out["justice_weighted_value"] = out["base_resilience_value"] * (0.72 + 0.028 * out["justice_protection"])

    conditions = [
        (out["base_resilience_value"] >= 7.35) & (out["maladaptation_risk"] <= 3.1),
        out["maladaptation_risk"] >= 3.7,
        out["justice_protection"] < 7.8,
        out["adaptive_capacity"] < 8.0,
    ]
    choices = [
        "strong climate resilience profile with manageable maladaptation risk",
        "maladaptation review needed",
        "justice protection needs strengthening",
        "adaptive capacity constraint",
    ]
    out["diagnostic"] = np.select(conditions, choices, default="promising but requires climate scenario validation")
    return out
...
def score_strategies(strategies: pd.DataFrame, scenario: pd.Series) -> pd.DataFrame:
    ...
    result["resilience_value"] = (
        ...
        + scenario["recovery_capacity_weight"] * result["recovery_capacity"]
        + scenario["transformative_capacity_weight"] * result["transformative_capacity"]
...
def simulate_system_response(system: pd.Series, events: pd.DataFrame, seed: int = 42, time_steps: int = 80) -> pd.DataFrame:
    ...
    threshold = system["threshold_proximity"]
    justice = system["justice_sensitivity"]
    ...
    for t in range(time_steps):
        ...
        adaptive_response = (
            0.42 * adaptive
            + 0.35 * recovery
            - 0.28 * system["exposure"]
            - 0.20 * system["vulnerability"]
        )
        adaptive_response = np.clip(adaptive_response, 0, 1)
        ...
        threshold = np.clip(threshold + 0.02 * stress_load - 0.012 * adaptive_response, 0, 1)

        justice_weighted_function = function_level * (0.72 + 0.28 * (1.0 - justice_burden + justice * 0.50))
        justice_weighted_function = np.clip(justice_weighted_function, 0, 1)
        ...
        rows.append(
            {
                ...
                "justice_weighted_function": justice_weighted_function,
            }
        )
...
def run_dynamic_simulations(systems: pd.DataFrame, events: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    summary = (
        ...
        .agg(
            ...
            mean_justice_weighted_function=("justice_weighted_function", "mean"),
            maximum_threshold_proximity=("threshold_proximity", "max"),
            final_threshold_proximity=("threshold_proximity", "last"),
        )
        .reset_index()
        .sort_values("mean_justice_weighted_function", ascending=False)
    )
...
def expand_training(strategies: pd.DataFrame, n: int = 2600) -> pd.DataFrame:
    ...
    for _ in range(n):
        ...
        value = (
            ...
            + 0.16 * record["vulnerability_reduction"]
            + 0.16 * record["adaptive_capacity"]
            + 0.15 * record["recovery_capacity"]
            + 0.15 * record["transformative_capacity"]
            + 0.14 * record["justice_protection"]
            - 0.08 * record["maladaptation_risk"]
            + rng.normal(0, 0.18)
        )

        record["resilience_value"] = value
        record["high_resilience_low_maladaptation"] = 1 if value >= 7.25 and record["maladaptation_risk"] <= 3.4 else 0
        rows.append(record)
...
def plot_outputs(
    ...
    plt.title("Climate Resilience Strategy Profiles")
    plt.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig(OUT_FIGURES / "climate_resilience_strategy_profiles.png", dpi=160)
    plt.close()
    ...
    plt.title("Climate Resilience Strategy Value Across Priorities")
    plt.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig(OUT_FIGURES / "climate_resilience_scenario_rankings.png", dpi=160)
    plt.close()
    ...
    plt.title("Climate System Function Under Repeated Stress")
    plt.legend(fontsize=7)
    plt.tight_layout()
    plt.savefig(OUT_FIGURES / "climate_system_function_under_stress.png", dpi=160)
    plt.close()

    plt.figure(figsize=(10, 6))
    plt.bar(dynamic_summary["system"], dynamic_summary["mean_justice_weighted_function"])
    plt.xticks(rotation=25, ha="right")
    plt.ylabel("Mean justice-weighted function")
    plt.title("Justice-Weighted Climate Resilience by System")
    plt.tight_layout()
    plt.savefig(OUT_FIGURES / "justice_weighted_climate_resilience_by_system.png", dpi=160)
    plt.close()
    ...
    plt.title("Robustness of Climate Resilience Strategies")
    plt.tight_layout()
    ...
    if importance is not None:
        ...
        plt.title("Feature Importance for Climate Resilience Classification")
        plt.tight_layout()
...
def main() -> None:
    ...
    profiles.to_csv(OUT_TABLES / "climate_resilience_strategy_profiles_advanced.csv", index=False)
    rankings.to_csv(OUT_TABLES / "climate_resilience_strategy_rankings_advanced.csv", index=False)
    dynamic_simulation.to_csv(OUT_TABLES / "climate_resilience_dynamic_simulation_advanced.csv", index=False)
    dynamic_summary.to_csv(OUT_TABLES / "climate_resilience_dynamic_summary_advanced.csv", index=False)
    strategy_simulation.to_csv(OUT_TABLES / "climate_resilience_monte_carlo_advanced.csv", index=False)
    robustness.to_csv(OUT_TABLES / "climate_resilience_robustness_summary_advanced.csv", index=False)

    training = expand_training(strategies)
    X = training[FEATURES]
    y = training["high_resilience_low_maladaptation"]
    ...
    metrics = pd.DataFrame(
        [{
            "model": "random_forest_climate_resilience_classifier",
    ...
    training.to_csv(OUT_TABLES / "synthetic_climate_resilience_training_data.csv", index=False)
    metrics.to_csv(OUT_TABLES / "advanced_climate_resilience_model_metrics.csv", index=False)
    importance.to_csv(OUT_TABLES / "advanced_feature_importance.csv", index=False)
    joblib.dump(model, OUT_MODELS / "climate_resilience_classifier.joblib")

    plot_outputs(profiles, rankings, dynamic_simulation, dynamic_summary, robustness, importance)

    print("Advanced climate resilience workflow complete.")
    print("Strategy profile summary:")
    print(profiles[["strategy", "base_resilience_value", "justice_weighted_value", "diagnostic"]].round(4))
    print("Model metrics:")
