...
def score(row, weights):
    ...
    equity_gap = max(0.0, 8.5 - x(row, "equity"))
    governance_gap = max(0.0, 8.5 - x(row, "governance_legitimacy"))
    recognition_gap = max(0.0, 8.5 - x(row, "recognition"))

    adjusted = value - 0.06 * equity_gap - 0.06 * governance_gap - 0.05 * recognition_gap
    return value, adjusted

def diagnostic(row):
    if x(row, "burden_shift") >= 5.0:
        return "burden-shifting review needed"
    if x(row, "equity") < 7.5:
        return "equity-performance review needed"
    if x(row, "governance_legitimacy") < 7.5:
        return "governance-legitimacy review needed"
    if x(row, "recognition") < 7.5:
        return "recognition review needed"
    if x(row, "implementation_burden") >= 4.0:
    ...
    for scenario in scenarios:
        ...
        for rank, (adjusted, raw, strategy) in enumerate(scored, start=1):
            rows.append({
                ...
                "rank": rank,
                "ethical_resilience_value": round(raw, 5),
                "adjusted_ethical_resilience_value": round(adjusted, 5),
...
def monte_carlo(strategies, weights, n=3000):
    ...
    for strategy in strategies:
        ...
        values = [float(r["adjusted_ethical_resilience_value"]) for r in subset]
        summary.append({
            "strategy_id": strategy["strategy_id"],
            "strategy": strategy["strategy"],
...
def main():
    strategies = read_csv(ROOT / "data/raw/ethical_resilience_strategies.csv")
    scenarios = read_csv(ROOT / "data/raw/ethical_resilience_priority_scenarios.csv")
    ...
    write_csv(OUT / "ethical_resilience_strategy_rankings_standard.csv", ranked)
    write_csv(OUT / "ethical_resilience_top_rank_summary_standard.csv", first_place_rows)
    write_csv(OUT / "ethical_resilience_uncertainty_simulation_standard.csv", simulation)
    write_csv(OUT / "ethical_resilience_robustness_summary_standard.csv", robustness)

    print("Ethical resilience strategy workflow complete.")
    print(f"Wrote outputs to: {OUT}")
    for row in first_place_rows:
        print(f"  {row['strategy']}: ranked first in {row['times_ranked_first']} ethical priority scenarios")
