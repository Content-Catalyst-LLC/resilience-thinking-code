...
def compute(df, weights):
    ...
    result["adjusted_resilience_value"] = result["resilience_value"] - 0.06 * result["governance_gap"] - 0.08 * result["equity_gap"]
    return result.sort_values("adjusted_resilience_value", ascending=False)

def main():
    strategies = pd.read_csv(ROOT / "data/raw/future_resilience_strategies.csv")
    scenarios = pd.read_csv(ROOT / "data/raw/future_resilience_priority_scenarios.csv")
    balanced = scenarios.loc[scenarios["scenario"] == "Balanced"].iloc[0]

    baseline = compute(strategies, balanced)
    baseline.to_csv(OUT_TABLES / "future_resilience_baseline_advanced.csv", index=False)
    ...
    for sim in range(n_simulations):
        ...
        for col in BENEFITS + PENALTIES:
            sampled[col] = rng.normal(strategies[col], 0.55).clip(1, 10)
    ...
    simulation.to_csv(OUT_TABLES / "future_resilience_uncertainty_simulation_advanced.csv", index=False)
    robustness.to_csv(OUT_TABLES / "future_resilience_robustness_summary_advanced.csv", index=False)
    ...
    plt.tight_layout()
    plt.savefig(OUT_FIGURES / "future_resilience_probability_ranked_first.png", dpi=160)
    plt.close()

    print("Advanced future resilience workflow complete.")
